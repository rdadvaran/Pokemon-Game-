...
def create_button(width, height, left, top, text_cx, text_cy, label, normal_color, hover_color):
    mouse_cursor = pygame.mouse.get_pos()
    button = Rect(left, top, width, height)
    if button.collidepoint(mouse_cursor):
        pygame.draw.rect(game, hover_color, button)
    else:
        pygame.draw.rect(game, normal_color, button)
    font = pygame.font.Font(pygame.font.get_default_font(), 16)
    text = font.render(f'{label}', True, black)
    text_rect = text.get_rect(center=(text_cx, text_cy))
    game.blit(text, text_rect)
...
# Welcome screen
game_status = 'welcome'
while game_status == 'welcome':
    game.fill((255, 197, 90))  # Set background color to "FFC55A"
    font = pygame.font.Font(pygame.font.get_default_font(), 30)
    text = font.render('Welcome to the Pokemon Game!', True, (0, 0, 0))
    text_rect = text.get_rect(center=(game_width // 2, game_height // 2 - 50))
    game.blit(text, text_rect)
    start_button = create_button(200, 50, game_width // 2 - 100, game_height // 2 + 50, game_width // 2, game_height // 2 + 75, 'Start', (255, 138, 8), (255, 101, 0))  # Set button colors
    pygame.display.update()
...
chat_text = [
    ...
    "Instructions:",
    "- Select your Pokemon and battle against rivals.",
    "- Use arrow keys to move and spacebar to interact.",
    "- Press 'A' to attack and 'P' to use potion.",
    "Are you ready to start?"
]
continue_button = create_button(200, 50, game_width // 2 - 100, game_height - 100, game_width // 2, game_height - 75, 'Continue', (91, 188, 255), (255, 250, 183))

typing_speed = 0.05  # Adjust the typing speed here
typed_text = ''
text_index = 0
start_time = time.time()
continue_button_visible = False

while game_status == 'chat':
    game.fill((154, 200, 205))  # Set background color to "9AC8CD"

    # Display chat text with typing effect
    if text_index < len(chat_text):
        elapsed_time = time.time() - start_time
        if elapsed_time >= typing_speed:
            typed_text += chat_text[text_index][len(typed_text)]
            start_time = time.time()
        if typed_text == chat_text[text_index]:
            text_index += 1
            typed_text = ''
    else:
    ...
    font = pygame.font.Font(pygame.font.get_default_font(), 16)
    y_offset = 50
    for i, line in enumerate(chat_text[:text_index]):
        text = font.render(line if i < text_index - 1 else typed_text, True, (0, 0, 0))
    ...
    if continue_button_visible:
        if continue_button.collidepoint(pygame.mouse.get_pos()):
            pygame.draw.rect(game, (255, 250, 183), continue_button)
        else:
            pygame.draw.rect(game, (91, 188, 255), continue_button)
        font = pygame.font.Font(pygame.font.get_default_font(), 16)
        text = font.render('Continue', True, (0, 0, 0))
        text_rect = text.get_rect(center=(game_width // 2, game_height - 75))
        game.blit(text, text_rect)
    ...
    for event in pygame.event.get():
        ...
        if event.type == MOUSEBUTTONDOWN:
            mouse_click = event.pos
            if continue_button_visible and continue_button.collidepoint(mouse_click):
